...
class Neo4jGraphProjection:
    ...
    def delete_owner(self, owner_user_id: str) -> None:
        try:
            with self.driver.session(database=self.database) as session:
                session.execute_write(self._delete_projection, owner_user_id)
            self._last_error = ""
        except Exception as error:
            self._last_error = str(error)
            raise
    ...
    @staticmethod
    def _delete_projection(tx: Any, owner_user_id: str) -> None:
        tx.run(
            "MATCH ()-[r:PROFILE_EDGE {projection_owner: $owner}]->() DELETE r",
            owner=owner_user_id,
        ).consume()
        tx.run(
            "MATCH (person:GraphEntity {entity_key: $person_key}) DETACH DELETE person",
            person_key=f"person:{owner_user_id}",
        ).consume()
        tx.run(
            "MATCH (n:GraphEntity) WHERE NOT (n)--() DELETE n"
        ).consume()
